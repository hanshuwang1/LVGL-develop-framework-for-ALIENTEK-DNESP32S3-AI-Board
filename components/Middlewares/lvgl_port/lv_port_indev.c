...
#ifdef CONFIG_LVGL_PORT_INDEV_USE_MOUSE_KEYPAD_ENCODER_BUTTON
...
#endif /* CONFIG_LVGL_PORT_INDEV_USE_MOUSE_KEYPAD_ENCODER_BUTTON */
...
static void touchpad_init(void)
{
    ...
    ESP_LOGI("lv_port_indev", "Initializing touchpad...");
    
    /* 确保LCD已经初始化，触摸驱动需要lcddev参数 */
    if (lcddev.panel_handle == NULL) {
        ESP_LOGE("lv_port_indev", "LCD not initialized! Please call lv_port_disp_init first.");
        while(1) {
    ...
    /* 确保扫描函数指针正确设置 */
    if (tp_dev.scan == NULL) {
        ESP_LOGE("lv_port_indev", "Touch scan function not set!");
    } 
}
...
static bool touchpad_is_pressed(void)
{
    ...
    // return false;
    bool pressed = (tp_dev.sta & TP_PRES_DOWN) ? true : false;
    ESP_LOGD("lv_port_indev", "Touch pressed: %s, sta=0x%04X", pressed ? "true" : "false", tp_dev.sta);
    return pressed;
}
...
static void touchpad_get_xy(int32_t * x, int32_t * y)
{
    ...
    // (*x) = 0;
    // (*y) = 0;
    ESP_LOGD("lv_port_indev", "Getting touch coordinates: x=%d, y=%d", tp_dev.x[0], tp_dev.y[0]);
    *x = tp_dev.x[0];
    *y = tp_dev.y[0];
}
...
#endif /* not using mouse keypad encoder button */
